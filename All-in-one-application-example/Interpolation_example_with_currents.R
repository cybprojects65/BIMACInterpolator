...
output_data<-"./temperature_argo_north_atlantic_interpolated_wt_currents.csv"
currents_u_file = "./ocean_currents/global_scale_currents_copernicus_eastward_05res.asc"
currents_v_file = "./ocean_currents/global_scale_currents_copernicus_northward_05res.asc"
exclude_zeros<-F
...
# Crop the global-scale ocean currents of Copernicus to the data bounding box
ru <- raster(currents_u_file)
cropped_raster_u <- crop(ru, extent(bbox))
NAvalue(cropped_raster_u) <- -9999
currents_u_file_temp<-"./tmp_crop_u.asc"
writeRaster(x = cropped_raster_u, filename = currents_u_file_temp, format = "ascii", overwrite = TRUE)
...
currents_v_file_temp<-"./tmp_crop_v.asc"
writeRaster(cropped_raster_v, currents_v_file_temp, format = "ascii", overwrite = TRUE)
...
output<-bimac_currents_depthboundaries(punctual_data_file,
                                       currents_u_file_temp,
                                       currents_v_file_temp,
...
file.remove(currents_u_file_temp)
file.remove(currents_v_file_temp)
